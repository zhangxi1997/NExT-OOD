...
class VidQADataset(Dataset):
    ...
    def __init__(self, video_feature_path, video_feature_cache, sample_list_path, vocab, use_bert, mode, task):
        ...
        self.vocab = vocab
        sample_list_file = osp.join(sample_list_path, 'NExT-QA/{}_parse.csv'.format(mode))
        self.sample_list = load_file(sample_list_file)
        self.video_feature_cache = video_feature_cache
        self.max_qa_length = 41
        self.use_bert = use_bert

        self.obj_action_top10 = json.load(open('dataset/NExT-OOD/object_action_Top10.json'))
        ...
        if self.use_bert:
            self.bert_file = osp.join(video_feature_path, 'qas_bert_single/{}_single.h5'.format(mode))
            print('use BERT single and without pre-train')
    ...
    def __getitem__(self, idx):
        ...
        for c in cate:
            action_top10 = self.obj_action_top10[str(c)]
            now_action = cur_sample['action'].split()[0]
            if now_action in action_top10:
                cate_flag = cate_flag * 0
