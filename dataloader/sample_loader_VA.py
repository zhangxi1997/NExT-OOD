...
class VidQADataset(Dataset):
    ...
    def __init__(self, video_feature_path, video_feature_cache, sample_list_path, vocab, use_bert, mode, task, balance_N):
        ...
        sample_list_file = osp.join(sample_list_path, 'NExT-OOD-VA/{}_VA_{}.csv'.format(mode, balance_N))
        ...
        if self.use_bert:
            self.bert_file = osp.join(video_feature_path, 'qas_bert_single/VA_{}_{}_single.h5'.format(balance_N, mode))
            print("------- Use reproduce bert features -------", balance_N)
