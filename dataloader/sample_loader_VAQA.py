...
class VidQADataset(Dataset):
    ...
    def __init__(self, video_feature_path, video_feature_cache, sample_list_path, vocab, use_bert, mode, task, balance_N):
        self.video_feature_path = video_feature_path
        self.vocab = vocab

        sample_list_file = osp.join(sample_list_path, 'NExT-OOD-VQA/{}_VQA_{}.csv'.format(mode,balance_N))

        self.sample_list = load_file(sample_list_file)
        self.video_feature_cache = video_feature_cache
        self.max_qa_length = 41
        self.use_bert = use_bert
        self.obj_action_top10 = ''
        ...
        if self.use_bert:
            self.bert_file = osp.join(video_feature_path, 'qas_bert_single/VAQA_{}_{}_single.h5'.format(balance_N, mode))
            print('qas_bert_single/VAQA_{}_{}_single.h5'.format(balance_N, mode))
            print("------- Use base and single bert features -------", balance_N)
    ...
    def __getitem__(self, idx):
        ...
        return video_feature, candidate_qas, qa_lengths, ans, qns_key, candidate_as, a_lengths , candidate_qs, q_lengths, cate_tensor, cate_flag
#
...
class QALoader():
    def __init__(self, batch_size, num_worker, video_feature_path, video_feature_cache,
                 sample_list_path, vocab, use_bert, task, balance_N, train_shuffle=True, val_shuffle=False):
    ...
    def validate(self, mode):
        ...
        validation_set = VidQADataset(self.video_feature_path, self.video_feature_cache, self.sample_list_path,
                                         self.vocab, self.use_bert, mode, self.task, self.balance_N)
