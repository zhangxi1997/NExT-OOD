...
def accuracy_metric(sample_list_file, result_file):
    ...
    # f_acc = open('results_balance_v3/K2_' + result_file.split('/')[-1], 'w')
    # json.dump(obj_group_Acc, f_acc)
    ...
    for key, value in preds.items():
        ...
        if answer == pred:
            acc += 1
        all += 1
    # print('acc / all accuracy:', round(acc/all, 4), acc, all)
...
def main(result_file, mode='val', balance_N='N1'):
    ...
    sample_list_file = osp.join(dataset_dir, 'NExT-OOD-VQA/' + data_set + '_VQA_' + balance_N + '.csv')
