...
class TempAttention_counter(nn.Module):
    """
    Counterfactual Temp Attention
    Applies an attention mechanism on the output features from the decoder.
    """

    def __init__(self, text_dim, visual_dim, hidden_dim):
        super(TempAttention_counter, self).__init__()
        self.hidden_dim = hidden_dim
        self.linear_text = nn.Linear(text_dim, hidden_dim)
        self.linear_visual = nn.Linear(visual_dim, hidden_dim)
        self.linear_att = nn.Linear(hidden_dim, 1, bias=False)
    ...
    def forward(self, qns_embed, vid_outputs):
        ...
        # ====  counterfactual part  ======
        e_random = torch.zeros_like(e).uniform_(0, 1)  # random [bs, 16]
        beta_random = F.softmax(e_random, dim=1)
        context_random = torch.bmm(beta_random.unsqueeze(1), vid_outputs).squeeze(1)
        context_counter = context - context_random #[bs, 512]
        # or concat ??

        return context, beta, context_counter

class TempAttention_counter2(nn.Module):
    """
    Counterfactual Temp Attention
    Applies an attention mechanism on the output features from the decoder.
    """
    ...
    def __init__(self, text_dim, visual_dim, hidden_dim):
        super(TempAttention_counter2, self).__init__()
        self.hidden_dim = hidden_dim
        self.linear_text = nn.Linear(text_dim, hidden_dim)
        self.linear_visual = nn.Linear(visual_dim, hidden_dim)
        self.linear_att = nn.Linear(hidden_dim, 1, bias=False)
        self.fc = nn.Linear(3*visual_dim, visual_dim)
        self._init_weight()
    ...
    def forward(self, qns_embed, vid_outputs):
        ...
        context_counter = torch.cat([context, context_random, context - context_random], dim=-1)
        # context_counter = context - context_random #[bs, 512]
        context_counter = self.fc(context_counter)

        return context, beta, context_counter


class TempAttention_counter2a(nn.Module):
    """
    Counterfactual Temp Attention
    Applies an attention mechanism on the output features from the decoder.
    """
    ...
    def __init__(self, text_dim, visual_dim, hidden_dim):
        super(TempAttention_counter2a, self).__init__()
        self.hidden_dim = hidden_dim
        self.linear_text = nn.Linear(text_dim, hidden_dim)
        self.linear_visual = nn.Linear(visual_dim, hidden_dim)
        self.linear_att = nn.Linear(hidden_dim, 1, bias=False)
...
class TempAttention_counter3(nn.Module):
    """
    Counterfactual Temp Attention
    Applies an attention mechanism on the output features from the decoder.
    """
    ...
    def __init__(self, text_dim, visual_dim, hidden_dim):
        super(TempAttention_counter3, self).__init__()
        self.hidden_dim = hidden_dim
        self.linear_text = nn.Linear(text_dim, hidden_dim)
        self.linear_visual = nn.Linear(visual_dim, hidden_dim)
        self.linear_att = nn.Linear(hidden_dim, 1, bias=False)
        self.fc = nn.Linear(visual_dim, visual_dim)
        self._init_weight()
    ...
    def forward(self, qns_embed, vid_outputs):
        ...
        beta = F.softmax(e, dim=1) #[bs, 16]
        context = torch.bmm(beta.unsqueeze(1), vid_outputs).squeeze(1)

        # ====  counterfactual part  ====== batch shuffle
        beta_shuffle = torch.zeros_like(beta)
        for i in range(beta.shape[0]):
            rand_idx = random.randint(0, beta.shape[0] - 1)
            while rand_idx == i:
                rand_idx = random.randint(0, beta.shape[0] - 1)
            beta_shuffle[i] = beta[rand_idx]
        context_random = torch.bmm(beta_shuffle.unsqueeze(1), vid_outputs).squeeze(1)
...
class TempAttention_counter4(nn.Module):
    """
    Counterfactual Temp Attention
    Applies an attention mechanism on the output features from the decoder.
    """
    ...
    def __init__(self, text_dim, visual_dim, hidden_dim):
        super(TempAttention_counter4, self).__init__()
        self.hidden_dim = hidden_dim
        self.linear_text = nn.Linear(text_dim, hidden_dim)
        self.linear_visual = nn.Linear(visual_dim, hidden_dim)
        self.linear_att = nn.Linear(hidden_dim, 1, bias=False)
...
class SpatialAttention_counter(nn.Module):
    """
    Apply spatial attention on vid feature before being fed into LSTM
    """

    def __init__(self, text_dim=1024, vid_dim=3072, hidden_dim=512, input_dropout_p=0.2):
        super(SpatialAttention_counter, self).__init__()
    ...
    def forward(self, qns_feat, vid_feats):
        ...
        vid_qns_o = vid_qns.view(fnum, region_num)
        alpha = self.softmax(vid_qns_o) #[16, 49]
        alpha = alpha.unsqueeze(1)
        vid_feats = vid_feats.view(fnum, region_num, -1)
        feature = torch.bmm(alpha, vid_feats).squeeze(1)
        feature = self.dropout(feature)
        ...
        # ---- counterfactual -----
        vid_qns_o_random = torch.zeros_like(vid_qns_o).uniform_(0, 1)
        alpha_random = self.softmax(vid_qns_o_random).unsqueeze(1)
        feature_random = torch.bmm(alpha_random, vid_feats).squeeze(1)
        feature_random = self.dropout(feature_random)
        feature_counter = feature - feature_random
        # [context, context_random, context - context_random]

        # print(feature.size())
        return feature, alpha, feature_counter


class SpatialAttention_counter2(nn.Module):
    """
    Apply spatial attention on vid feature before being fed into LSTM
    """

    def __init__(self, text_dim=1024, vid_dim=3072, hidden_dim=512, input_dropout_p=0.2):
        super(SpatialAttention_counter2, self).__init__()

        self.linear_v = nn.Linear(vid_dim, hidden_dim)
        self.linear_q = nn.Linear(text_dim, hidden_dim)
        self.linear_att = nn.Linear(hidden_dim, 1, bias=False)
        self.fc = nn.Linear(3*4096, 4096)
    ...
    def forward(self, qns_feat, vid_feats):
        ...
        feature_counter = torch.cat([feature, feature_random, feature-feature_random], dim=-1)
        feature_counter = self.fc(feature_counter)
        # feature_counter = feature - feature_random
        # [context, context_random, context - context_random]
        #
        # print(feature.size())
...
class SpatialAttention_counter3(nn.Module):
    """
    Apply spatial attention on vid feature before being fed into LSTM
    """

    def __init__(self, text_dim=1024, vid_dim=3072, hidden_dim=512, input_dropout_p=0.2):
        super(SpatialAttention_counter3, self).__init__()
    ...
    def forward(self, qns_feat, vid_feats):
        ...
        # ---- counterfactual -----
        # ====  counterfactual part  ====== batch shuffle
        alpha_shuffle = torch.zeros_like(alpha)
        for i in range(alpha.shape[0]):
            rand_idx = random.randint(0, alpha.shape[0] - 1)
            while rand_idx == i:
                rand_idx = random.randint(0, alpha.shape[0] - 1)
            alpha_shuffle[i] = alpha[rand_idx]
        feature_random = torch.bmm(alpha_shuffle, vid_feats).squeeze(1)
        feature_random = self.dropout(feature_random)
