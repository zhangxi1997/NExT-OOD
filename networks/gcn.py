...
class AdjLearner_intra(Module):
    ...
    def forward(self, questions, videos):
        ...
        '''
        # graph_nodes (batch_size, in_feat_dim): input features
        # graph_nodes = torch.cat((questions, videos), dim=1)
        graph_nodes1 = questions # [bs, 256]

        # layer 1
        h1 = self.edge_layer_1(graph_nodes1)
        h1 = F.relu(h1)

        # layer 2
        h1 = self.edge_layer_2(h1)
        h1 = F.relu(h1)
        # h * sigmoid(Wh)
        # h = F.tanh(h)
        ...
        # layer 1
        h2 = self.edge_layer_1(graph_nodes2)
        h2 = F.relu(h2)

        # layer 2
        h2 = self.edge_layer_2(h2)
        h2 = F.relu(h2)
        # h * sigmoid(Wh)
        # h = F.tanh(h)

        # outer product
        adjacency_matrix = torch.mm(h1, h2.transpose(0, 1))

        return adjacency_matrix

class AdjLearner_inter(Module):
    def __init__(self, in_feature_dim, hidden_size, dropout=0.1):
        super().__init__()
        '''
        ## Variables:
        ...
        self.edge_layer_1b = nn.Linear(in_feature_dim, hidden_size, bias=False)
        self.edge_layer_2b = nn.Linear(hidden_size, hidden_size, bias=False)

        # Regularisation
        self.dropout = nn.Dropout(p=dropout)
        self.edge_layer_1b = nn.utils.weight_norm(self.edge_layer_1b)
        self.edge_layer_2b = nn.utils.weight_norm(self.edge_layer_2b)
    ...
    def forward(self, questions, videos):
        ...
        graph_nodes2 = videos
        # layer 1
        h2 = self.edge_layer_1b(graph_nodes2)
        h2 = F.relu(h2)

        # layer 2
        h2 = self.edge_layer_2b(h2)
        h2 = F.relu(h2)
        # h * sigmoid(Wh)
        # h = F.tanh(h)
...
class AdjLearner_intra_simple(Module):
    def __init__(self, in_feature_dim, hidden_size, dropout=0.1):
        super().__init__()
        '''
        ## Variables:
