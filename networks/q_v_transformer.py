...
class CoAttentionLayer(nn.Module):
    ...
    def forward(
        ...
        return output_q, output_v, output_q, output_v
...
class CoAttentionLayer_counter(nn.Module):
    ...
    def forward(
        ...
        output_q_counter = self.layer_norm_qv(output_qv_counter + q)
        ...
        output_v_counter = self.layer_norm_vq(output_vq_counter + v)

        return output_q, output_v, output_q_counter, output_v_counter

class CoAttentionLayer_counter2(nn.Module):
...
class CoAttention(nn.Module):
    ...
    def __init__(self, hidden_size, n_layers=1, dropout_p=0.0):
        ...
        self.encoder_layers = nn.ModuleList(
            [CoAttentionLayer_counter2(hidden_size, dropout_p) for _ in range(n_layers)])
...
class CoAttention_init(nn.Module):
    ...
    def forward(self, question, video):
        ...
        return question, video, _, _

class CoAttention_nlayer_counter(nn.Module):
    ...
    def forward(self, question, video):
        ...
        for layer, encoder in enumerate(self.encoder_layers):
            if layer == 0:
                question, video, question_counter, video_counter = encoder(
                    question,
                    video,
                ...
                question, video, _, _ = encoder(
                    ...
                    attn_mask_=attn_mask_,
                    softmax_mask_=softmax_mask_)
                _, _, question_counter, video_counter = encoder(
                    question_counter,
                    video_counter,
                    attn_mask=attn_mask,
                    softmax_mask=softmax_mask,
